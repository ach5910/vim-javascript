...
syntax region  jsClassBlock         contained matchgroup=jsClassBraces         start=/{/  end=/}/  contains=@jsAll,jsClassFuncName,jsClassArrowFuncName,jsClassMethodType,jsArrowFunction,jsArrowFuncArgs,jsComment,jsGenerator,jsDecorator,jsClassProperty,jsClassPropertyComputed,jsClassStringKey,jsAsyncKeyword,jsNoise extend fold
...
syntax match   jsDot            "\v\?\." skipwhite skipempty nextgroup=jsObjectProp,jsFuncCall,jsPrototype,jsTaggedTemplate
syntax match   jsObjectProp     contained /\<\K\k*/
" syntax match   jsClassContructorCall /new\s*\zs\<[A-Z]\k*\ze\s*(/
syntax match   jsClassContructorCall contained /\<[A-Z]\k*\ze\s*(/ skipwhite skipempty
syntax match   jsFuncCall       /\<\K\k*\ze\s*(/
syntax match   jsParensError    /[)}\]]/

" Program Keywords
syntax keyword jsStorageClass   const var let skipwhite skipempty nextgroup=jsDestructuringBlock,jsDestructuringArray,jsArrowFuncName,jsVariableDef
syntax match   jsVariableDef    contained /\<\K\k*/ skipwhite skipempty nextgroup=jsFlowDefinition
syntax keyword jsOperatorWord   delete instanceof typeof void in of skipwhite skipempty nextgroup=@jsExpression
syntax keyword jsOperatorWord   new skipwhite skipempty nextgroup=jsClassContructorCall
syntax match   jsOperator       "[-!|&+<>=%/*~^]" skipwhite skipempty nextgroup=@jsExpression
syntax match   jsOperator       /::/ skipwhite skipempty nextgroup=@jsExpression
...
syntax keyword jsExportDefault      contained default skipwhite skipempty nextgroup=@jsExpression,jsModule
syntax keyword jsExportDefaultGroup contained default skipwhite skipempty nextgroup=jsModuleAs,jsFrom,jsModuleComma,jsModule
syntax match   jsModule             contained /\<\K\k*\ze;/ skipwhite skipempty
...
" syntax region  jsParenCatch         contained matchgroup=jsParensCatch         start=/(/  end=/)/  skipwhite skipempty nextgroup=jsTryCatchBlock extend fold
" syntax region  jsFuncArgs           contained matchgroup=jsFuncParens          start=/(/  end=/)/  contains=jsFuncArgCommas,jsComment,jsFuncArgExpression,jsDestructuringBlock,jsDestructuringArray,jsRestExpression,jsFlowArgumentDef skipwhite skipempty nextgroup=jsCommentFunction,jsFuncBlock,jsFlowReturn extend fold
" syntax region  jsClassBlock         contained matchgroup=jsClassBraces         start=/{/  end=/}/  contains=jsClassFuncName,jsClassArrowFuncName,jsClassMethodType,jsArrowFunction,jsArrowFuncArgs,jsComment,jsGenerator,jsDecorator,jsClassProperty,jsClassPropertyComputed,jsClassStringKey,jsAsyncKeyword,jsNoise extend fold
" syntax region  jsFuncBlock          contained matchgroup=jsFuncBraces          start=/{/  end=/}/  contains=@jsAll,jsBlock extend fold
" syntax region  jsIfElseBlock        contained matchgroup=jsIfElseBraces        start=/{/  end=/}/  contains=@jsAll,jsBlock extend fold
" syntax region  jsTryCatchBlock      contained matchgroup=jsTryCatchBraces      start=/{/  end=/}/  contains=@jsAll,jsBlock skipwhite skipempty nextgroup=jsCatch,jsFinally extend fold
" syntax region  jsFinallyBlock       contained matchgroup=jsFinallyBraces       start=/{/  end=/}/  contains=@jsAll,jsBlock extend fold
...
syntax match   jsArrowFuncName        contained /\<\K\k*\ze\s*=\s*([^()]*)\s*=>/ skipempty skipwhite nextgroup=jsArrowFuncArgs
syntax match   jsArrowFuncName        contained /\<\K\k*\ze\s*=\s*=>/ skipempty skipwhite nextgroup=jsArrowFuncArgs
...
syntax match   jsClassFuncName          contained /\<\K\k*\ze\s*[=(<]/ skipwhite skipempty nextgroup=jsFuncArgs,jsFlowClassFunctionGroup,jsArrowFuncArgs
syntax match   jsClassMethodType        contained /\<\%([gs]et\|static\)\ze\s\+\K\k*/ skipwhite skipempty nextgroup=jsAsyncKeyword,jsClassFuncName,jsClassArrowFuncName,jsClassProperty
...
syntax match   jsClassArrowFuncName    contained /\<\K\k*\ze\s*=\s*([^()]*)\s*=>/ skipempty skipwhite nextgroup=jsArrowFuncArgs 
syntax match   jsClassArrowFuncName    contained /\<\K\k*\ze\s*=\s*=>/ skipempty skipwhite nextgroup=jsArrowFuncArgs 
...
syntax match   jsObject          /\<\K\k*\ze\./ nextgroup=jsDot
syntax match   jsObject          /\<\K\k*\ze?/ nextgroup=jsDot
syntax match   jsArray          /\<\K\k*\ze\[/ 
...
syntax cluster jsExpression  contains=jsBracket,jsParen,jsObjectBlock,jsTernaryIf,jsTaggedTemplate,jsTemplateString,jsString,jsRegexpString,jsNumber,jsFloat,jsOperator,jsOperatorWord,jsBooleanTrue,jsBooleanFalse,jsNull,jsFunction,jsArrowFunction,jsGlobalObjects,jsExceptions,jsFutureKeys,jsDomErrNo,jsDomNodeConsts,jsHtmlEvents,jsObject,jsArray,jsClassContructorCall,jsFuncCall,jsUndefined,jsNan,jsPrototype,jsBuiltins,jsNoise,jsClassDefinition,jsArrowFunction,jsArrowFuncArgs,jsParensError,jsComment,jsArguments,jsThis,jsSuper,jsDo,jsForAwait,jsAsyncKeyword,jsStatement,jsDot
...
if version >= 508 || !exists("did_javascript_syn_inits")
  ...
  HiLink jsArrowFuncName        jsFuncName
  HiLink jsClassArrowFuncName        jsFuncName
